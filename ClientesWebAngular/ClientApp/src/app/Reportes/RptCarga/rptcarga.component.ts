...
@Component({
  ...
  templateUrl: './rptcarga.component.html',
  styleUrls: ['./rptcarga.component.html'],
})
export class RptCargaComponent implements OnInit, OnDestroy, AfterViewInit {
  paises: IPais[] = [];
  ...
  selectedEjecutivos: any;
  selectedEjecutivosStr = "";
  ...
  async onChange(deviceValue) {
    ...
    if (deviceValue != -1) {
      ...
      if (respuesta.success) {
        ...
        //Precargamos el objeto de ejecutivos
        var ejec = obj.ejecutivos
        if (ejec != "") {
          var objEjec = ejec.split(",");
          var ejecutivosEncontrados: any;
          ejecutivosEncontrados = this.buscarTextoModel(objEjec, this.ejecutivos).split(",");
          const index = ejecutivosEncontrados.indexOf("");
          if (index > -1) {
            ejecutivosEncontrados.splice(index, 1);
          }
          this.selectedEjecutivos = ejecutivosEncontrados;
          this.selectedEjecutivosStr = this.buscarIdModel(objEjec, this.ejecutivos);
        }
  ...
  getValuesConocio(event): void {
    ...
    for (var i = 0; i < event.length; i++) {
      if (this.selectedConocioStr == "") {
        if (i < event.length - 1) {
          this.selectedConocioStr = this.selectedConocioStr + event[i].id + ",";
        }
        else {
          this.selectedConocioStr = this.selectedConocioStr + event[i].id;
        }
  ...
  seleccionarReporte(rpt): void {

    this.reporteSeleccionado = 0;
    var obj = JSON.parse(rpt.contenido);
    this.rptoboxObject = obj;
  ...
  tarjetaFormDos = new FormGroup({
    ...
    rptEmpresas: new FormControl(),
    Ejecutivos: new FormControl(),
  });
    ...
    if (this.selectedColumnasStr != "") {
      ...
      if (this.userNameCookie === '' || UtilApp.isNullOrUndefined(this.userNameCookie) || this.user.token === '') {
        ...
        var data = {
          ...
          "ejecutivos": this.selectedConocioStr,
        ...
        this.url = AppConfigService.settings.server + 'reportes/rcarga-generar';
        this.jdata = { jdata: data, jSessionId: this.user.token };
        this.data = 'jdata=' + encodeURIComponent(JSON.stringify(this.jdata));
        const httpOptions = {
          headers: new HttpHeaders({
