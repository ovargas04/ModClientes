...
export class CambioClaveComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  async actualizar_pressed() {
    try {
      debugger;
      var actual = this.cambioClaveForm.controls['actual'].value;
      var nueva = this.cambioClaveForm.controls['nueva'].value;
      var confirmacion = this.cambioClaveForm.controls['confirmacion'].value;


      if (actual == null) {
        this.mostrarMensaje("<p>Debe digitar la contraseña actual.</p>", this.iconoError);
      }
      else
        if (nueva == null) {
          this.mostrarMensaje("<p>Debe digitar la contraseña nueva.</p>", this.iconoError);
        }
        else if (nueva != confirmacion) {
          this.mostrarMensaje("<p>Las contraseñas no coinciden.</p>", this.iconoError);
        }
        else {
          ...
          let usuarioRespuesta = await this.llamadaGenerica(data, "cambiando clave...", "usuario/cambio-clave");
          if (usuarioRespuesta.success) {
            this.mostrarMensaje("<p>Se ha realizado el cambio de contraseña correctamente.</p>", this.iconoSuccess);
          } else {
            this.mostrarMensaje("<p>No se han podido realizar el cambio de contraseña. Por favor intente de nuevo</p>", this.iconoError);
          }
        }
    }
