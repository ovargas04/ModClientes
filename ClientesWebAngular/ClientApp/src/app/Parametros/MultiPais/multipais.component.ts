...
@Component({
  selector: 'app-multipais',
  templateUrl: './multipais.component.html',
  styleUrls: ['./multipais.component.html'],
})
export class MultipaisComponent implements OnInit, OnDestroy, AfterViewInit {
  paises: IPais[] = [];
  ...
  ngAfterViewInit(): void {
    ...
    this.cargarDropDown("#contratoAereoSelect", "parametros/contratos", { value: 0, text: 1, secondText: null }, "0", "Seleccione Contrato Aéreo");
    this.cargarDropDown("#contratoEconomySelect", "parametros/contratos", { value: 0, text: 1, secondText: null }, "0", "Seleccione Contrato Economy");
    this.cargarDropDown("#contratoGoldSelect", "parametros/contratos", { value: 0, text: 1, secondText: null }, "0", "Seleccione Contrato Gold");
    this.cargarDropDown("#productoPOBoxSelect", "parametros/productos", { value: 0, text: 1, secondText: null }, "0", "Seleccione Producto PO Box");
    this.cargarDropDown("#productoAgenciaAduanalSelect", "parametros/productos", { value: 0, text: 1, secondText: null }, "0", "Seleccione Producto Aduanal");
    this.cargarDropDown("#manejoDefaultSelect", "parametros/manejo", { value: 0, text: 1, secondText: null }, "0", "Seleccione Tabla Manejo");
  }
...
/*********CARGA DE DATA*************/
  ...
  async cargarMultipais()
  {
    ...
    if (this.userNameCookie === '' || UtilApp.isNullOrUndefined(this.userNameCookie) || this.user.token === '') {
      ...
      this.url = AppConfigService.settings.server + 'config/multipais';
  ...
  multiPaisEliminar: number = -1;
  async LevantarPopUpEliminar(request)
  {
    this.multiPaisEliminar = request.id;
    this.triggerClick("btnEliminarMultiPais");
  }

  async eliminarMultiPais()
  {
    if (this.multiPaisEliminar < 0) {
    ...
    let respuestaEliminado = await this.llamadaGenerica({ "method": "DELETE", "codigoPais": this.multiPaisEliminar }, "eliminando...", "parametros/mensajero-borrar");
    ...
    if (respuestaEliminado.success) {
      ...
      await this.mostrarMensaje("<p>No se pudo borrar el país, esta siendo usado!</p>", this.iconoError);
    }
  ...
  async actualizarMultiPais() {
    try {
      ...
      $("#ContentModal select,#ContentModal input,#ContentModal textarea").each(function () {
        let elemento = $(this);
        let value = null;
        try {
      ...
      if (data["pais"].trim() == "") {
        await this.mostrarMensaje("<p>Favor indicar el País</p>", this.iconoError);
        return;
      }
      if (data["suffix"].trim() == "") {
        await this.mostrarMensaje("<p>Favor indicar el Código del País</p>", this.iconoError);
        return;
      }
      if (data["division0"].trim() == "" || data["division1"].trim() == "") {
        await this.mostrarMensaje("<p>Favor indicar el texto para división y sub-división</p>", this.iconoError);
        return;
      }
      if (data["cedula-juridica"].trim() == "") {
        await this.mostrarMensaje("<p>Favor indicar el texto fiscal: cedula juridica, RUC, etc</p>", this.iconoError);
        return;
      }


      var resultado = await this.llamadaGenerica(data, "guardando datos...", "config/multipais-actualizar");
